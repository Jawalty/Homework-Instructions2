...
- Creating and interpreting histograms with different bindwidths
...
The graph has the highest count between the 30-60k range, being right skewed. The median income is 36,000. The spread runs approximately 33000-45000 with a value of 11470.18, and there are a few outliers ahead of the 90,000 range.
...
6.  Which major category has the highest typical (you'll need to decide what this means) median income? Use the partial code below, filling it in with the appropriate statistic and function. Also note that we are looking for the highest statistic, so make sure to arrange in the correct direction. Engineering with a median of 57,000.
...
7.  Which major category is the least popular in this sample? To answer this question we use a new function called `count`, which first groups the data and then counts the number of observations in each category (see below). Add to the pipeline appropriately to arrange the results so that the major with the lowest observations is on top. Interdisciplinary is the lowest with a value of 1.
...
college_recent_grads %>%
  filter(
    major_category %in% c("Computers & Mathematics", "Engineering", 
...
There is unfortunately a somewhat downward trend on the graph where some of the fields with the highest proportion of women also have some of the lowest median incomes in comparison to the others. There are some noticeably strong fields like computer, mathematics, and law that have high median incomes while still having strong proportions of women.

```{r women-income-stem, eval = TRUE}
ggplot(data = college_recent_grads, aes(x = sharewomen, y = median, color = major_category)) +
  geom_point() +
  labs(
    x = "Proportion of Women",
    y = "Median Income",
    color = "Major Category",
    title = "Median Income vs. Proportion of Women by Major"
  )
```
...
Which majors have the strongest and weakest mean employment rates?
...
college_recent_grads %>%
  ...
  summarise(
    ...
    mean_unemployed = mean(unemployment_rate, na.rm = TRUE)
  ) %>%
  arrange(desc(mean_employed))
```

**Your findings (3-4 sentences):**
Business and Communications have the strongest mean employment rates across the board. They both are in the 80,000 range. Agriculture and Interdisciplinary are the lowest in mean employment rates, both being under 10,000.
